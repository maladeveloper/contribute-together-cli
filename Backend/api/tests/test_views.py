...
class TaxLatest(TestCase):
    def create_models(self):
        ...
        latest_interval  = Interval.objects.create(start_date='2021-10-04', end_date='2021-10-17')
        ...
        #User 0 has income in interval directly before latest interval
        before_income0 = Income.objects.create(incomesource_id=income_source0.id, amount=2000, date='2021-09-23')
        #All other users have income in latest interval
        income0 = Income.objects.create(incomesource_id=income_source0.id, amount=509, date='2021-10-07')
        income1 = Income.objects.create(incomesource_id=income_source1.id, amount=689, date='2021-10-07')
    ...
    def test_with_all_user_incomes(self):
        ...
        self.assertEqual(response.data, {'TEST000': 1023, 'TEST001': 77})
...
class IncomePerInterval(TestCase):
    def create_models(self):
        ...
        income3 = Income.objects.create(incomesource_id=income_source3.id, amount=123, date='2021-10-07')
        return target_interval 

    def test_when_all_users_have_income_in_interval(self):
        target_interval = self.create_models()
        response = client.get('/api/income/income-source/' + str(target_interval.id), follow=True)
        self.assertEqual(response.data, {'TEST000': {'TestIncomeSource': 1000}, 'TEST001': {'TestIncomeSource': 500}, 'TEST002': {'TestIncomeSource': 500, 'AnotherIncomeSource': 123}})

class IncomeAvgPerInterval(IncomePerInterval):
    def test_avg_income_per_interval_when_all_users_paid(self):
        target_interval = self.create_models()
        response = client.get('/api/income/averaged/' + str(target_interval.id), follow=True)
        self.assertEqual(response.data,{'TEST000': 500, 'TEST001': 250, 'TEST002': 311})
